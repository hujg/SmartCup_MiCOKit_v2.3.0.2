...
mico_spi_device_t mico_spi_SD =
{
    .port        = MICO_SPI_1,
    .chip_select = SD_PIN_SPI_CS,
    .speed       = 20000000,
...
void SD_SPI_SpeedLow(void)
{
 	...
 	mico_spi_SD.speed = 400000;
    MicoSpiInitialize( &mico_spi_SD);
}
//SD卡正常工作的时候,可以高速了
void SD_SPI_SpeedHigh(void)
{
 	//SPI1_SetSpeed(SPI_BaudRatePrescaler_2);//设置到高速模式
 	mico_spi_SD.speed = 40000000;// 40000000;
    MicoSpiInitialize( &mico_spi_SD);
}
//SPI硬件层初始化
u8  SD_SPI_Init(void)
{
    ...
    if ( kNoErr != MicoSpiInitialize( &mico_spi_SD) )
    {
